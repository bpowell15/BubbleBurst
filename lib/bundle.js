  ...
  _createClass(Game, [{
    ...
    value: function drawGame() {
      ...
      if (this.board.shotBubbles.length !== 0) {
        this.board.shotBubbles.forEach(function (bubble, index) {
          _this.moveShotBubble(bubble, index);
          _this.bubbleCollision(bubble, index);
          bubble.drawBubble();
          if (_this.finishedShots.length > 0) {
            _this.removeCollidedShots(bubble);
          }
        });
      }

      this.board.bubbles[31].forEach(function (bubble) {
        if (bubble.type === 1) {
          alert("Game Over");
        }
      });
    ...
    value: function removeCollidedShots(bubble) {
      ...
      debugger;
      this.finishedShots.forEach(function (idx) {
        if (bubble.y === 10) {
          _this2.board.bubbles[_this2.board.bubbles.length - 1].push(bubble);
        }
        _this2.board.shotBubbles.splice(idx, 1);
      });
      this.finishedShots = [];
    }
  }, {
    key: 'moveShotBubble',
    value: function moveShotBubble(bubble, index) {
    ...
    key: 'bubbleCollision',
    value: function bubbleCollision(bubble, index) {
      for (var r = 0; r < this.board.bubbles.length; r++) {
        ...
        for (var c = 0; c < this.board.bubbles[r].length; c++) {
          ...
          if (this.board.bubbles[r][c].x - bubble.x > -20 && this.board.bubbles[r][c].x - bubble.x <= 0 && this.board.bubbles[r][c].y - bubble.y > -20 && this.board.bubbles[r][c].y - bubble.y <= 0) {
            debugger;
            ...
            this.finishedShots.push(index);
            leave = true;
          } else if (this.board.bubbles[r][c].x - bubble.x < 20 && this.board.bubbles[r][c].x - bubble.x >= 0 && this.board.bubbles[r][c].y - bubble.y < 20 && this.board.bubbles[r][c].y - bubble.y >= 0) {
            if (this.board.bubbles[r + 1][c].type === 0) {
              bubble.pos = { c: c, r: r + 1 };
              bubble.x = this.board.bubbles[r + 1][c].x;
              bubble.y = this.board.bubbles[r + 1][c].y;
    ...
    value: function loadNextRow() {
      ...
      if (this.nextRow.length !== BOARDOPTIONS.columnCount) {
        for (var c = 0; c < BOARDOPTIONS.columnCount; c++) {
          ...
          this.nextRow.push(new _bubble2.default(bubbleX, 10, this.ctx, 1));
        }
      }
    }
  }, {
    ...
    value: function createBubbles() {
      ...
      for (var r = 0; r < BOARDOPTIONS.nullRowCount; r++) {
        ...
        for (var c = 0; c < BOARDOPTIONS.columnCount; c++) {
          ...
          if (r < 8) {
            bubbles[r].push(new _bubble2.default(bubbleX, bubbleY, this.ctx, 1, { c: c, r: r }));
          } else {
            bubbles[r].push(new _bubble2.default(bubbleX, bubbleY, this.ctx, 0, { c: c, r: r }));
          }
        }
      }
